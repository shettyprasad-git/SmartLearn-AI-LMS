    ...
    <div className="p-8 md:p-12 space-y-12 max-w-7xl mx-auto pb-32">
       ...
       <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           {[
             { label: "Global Progress", value: `${data?.globalProgress || 0}%`, icon: <Target />, color: "text-primary", bg: "bg-primary/10" },
             { label: "Learning Hours", value: data?.learningHours || "0", icon: <Clock />, color: "text-blue-400", bg: "bg-blue-400/10" },
             { label: "Courses Enrolled", value: data?.enrolledCount || 0, icon: <BookOpen />, color: "text-purple-400", bg: "bg-purple-400/10" },
             { label: "Milestones", value: data?.completedCount || 0, icon: <Award />, color: "text-emerald-400", bg: "bg-emerald-400/10" }
           ].map((stat, i) => (
              <motion.div
                key={i}
           ...
           <div className="lg:col-span-2 glass-premium p-10 rounded-[56px] border-white/5 space-y-10">
              ...
              <div className="space-y-8">
                 ...
                 {(!data?.activeCourses || data.activeCourses.length === 0) && (
                   <p className="text-center py-10 text-muted-foreground text-xs uppercase tracking-widest font-bold">No active curriculums detected.</p>
                 )}
              </div>
           </div>
              ...
              <div className="py-12 flex flex-col items-center">
                 <div className="relative">
                    <div className="w-32 h-32 rounded-full border-4 border-white/5 flex items-center justify-center">
                       <span className="text-5xl font-black text-white italic tracking-tighter">{data?.streak || 0}</span>
                    </div>
